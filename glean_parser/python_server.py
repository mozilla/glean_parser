...
* Events as `Event` metric type, where we generate a `ServerEventLogger` class per ping with
  ...
  applications as it allows full leverage of standard Data Platform tools post-ingestion
  (Glean Dictionary, autogenerated dashboards).
* Custom pings-as-events, where for each ping we generate a class with a generic `record()`
  method that accepts an events list, usually with an `event_name` string metric.
"""
